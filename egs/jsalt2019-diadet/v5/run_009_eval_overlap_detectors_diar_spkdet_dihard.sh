...
out_dir=${exp_dir}/test_raw_DIHARD-MAY27
config_overlap=config.yml
vaddir_ov=`pwd`/vad_ov  # VAD without OV regions

# SRI overlap model is trained in CHiME5
tst_vec=(DIHARD_dev.SpeakerDiarization.All DIHARD_eval.SpeakerDiarization.All)
trn_vec=(AMI.SpeakerDiarization.MixHeadset AMI.SpeakerDiarization.MixHeadset)
val_vec=(AMI.SpeakerDiarization.MixHeadset.development AMI.SpeakerDiarization.MixHeadset.development)
