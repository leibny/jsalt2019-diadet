...
# dsets=`find $score_dir  -maxdepth 1 -name "jsalt19_spkdiar*" \
#   | xargs -l basename \
#   | sort \
#   | grep -v VB \
#   `
dsets="jsalt19_spkdiar_ami_dev_Mix-Headset jsalt19_spkdiar_ami_eval_Mix-Headset"
# dsets="jsalt19_spkdiar_ami_eval_Mix-Headset"
...
if [ $stage -le 1 ]; then
	...
	for dset in $dsets; do
		...
		# The VB resegmentation dir must exist to perform overlap assignment 
		# -- we need the q matrices
		dset_vb=${dset}_${vb_suff}
		if [ ! -d ${score_dir}/${dset_vb} ]; then 
			continue
		fi
		
		# This is the dir that the overlap assignment will use 
		output_dir=${score_dir}/${dset_vb}_${ovl_suff}
		...
		rm -rf $ouutput_dir/result.*
		rm -rf $output_dir/rttm

		# get rttm from BEFORE VB reseg
		# cp $score_dir/$dset/plda_scores_tbest/rttm $output_dir/vad.rttm
		cp $score_dir/$dset/rttm $output_dir/vad.rttm
		
		# overlap_rttm=${ovl_out_dir}/diar_overlap_${corp}_${part}${mic}.rttm
		# overlap_rttm=$output_dir/overlap.rttm
		...
		# TESTING these should be copied
		cp $score_dir/$dset_vb/tmp/*.npy $output_dir/q_mats
		...
		$train_cmd $output_dir/overlap_assignment.log \
...
ovl_dsets="jsalt19_spkdiar_ami_dev_Mix-Headset_VB_OVLassign jsalt19_spkdiar_ami_eval_Mix-Headset_VB_OVLassign"
# ovl_dsets="jsalt19_spkdiar_ami_dev_Mix-Headset_VB_OVLassign"
